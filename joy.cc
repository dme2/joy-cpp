 ...
 *  [] garbage collection ? (joy doesn't really use variables in the same way as
 most langs, so this might not be necessary. we just have to clean the stack out
           when necessary)
...
struct joy_object {
  ...
  joy_object(std::vector<joy_object *> ob_list)
      : list_data(ob_list), type(LIST) {}
  joy_object(Type t) : type(t) {}
  joy_object(bool b) : data((void *)b), type(BOOL) {}
  joy_object(const joy_object &o)
      : type(o.type), data(o.data), float_data(o.float_data),
...
int64_t get_int(joy_object *o) { return (int64_t)o->data; }

float get_float(joy_object *o) { return o->float_data; }

std::string get_string(joy_object *o) { return o->string_data; }

// N.B. chars are represented via strings
std::string get_char(joy_object *o) { return o->string_data; }

bool get_bool(joy_object *o) { return (bool)o->data; }

std::vector<joy_object *> get_list(joy_object *o) { return o->list_data; }

std::vector<joy_object *> get_set(joy_object *o) { return o->list_data; }

std::string get_op(joy_object *o) { return o->ident; }
...
uint8_t cur_stack_size() { return STACK_SIZE - stack_ptr; }
...
joy_object *op_get_head() { return stck[stack_ptr]; }
...
joy_object *op_uncons() {
  ...
  if (a->type == STR) {
    ...
    auto res = new joy_object(l_a[1]);
    auto res2 =
        new joy_object(std::vector<joy_object *>(l_a.begin() + 1, l_a.end()));
    res->type = l_a[1]->type;
    op_pop();
    op_push(res, res->type);
    op_push(res2, LIST);
...
joy_object *op_id() { return nullptr; }
...
joy_object *op_append() {
  ...
  op_pop();
  op_pop();
  auto res_obj = new joy_object(l_a);
  op_push(res_obj, LIST);
...
void op_comb_nullary() { op_comb_i(); }
...
void setup_builtins() {
  ...
  builtins["append"] = (voidFunction)op_append;
...
/** PARSER **/
bool peek(std::string::const_iterator i, std::string *input,
          std::string match) {
  // TODO: fix this
  while (i != input->end()) {
    if (!std::isspace(*i))
...
std::string::const_iterator eat_space(std::string *input,
...
// unused
std::string::const_iterator parse_op(std::string::const_iterator i,
                                     std::string *input) {
  std::string cur_op;
...
parse_ident(std::string::const_iterator i, std::string *input,
            bool exec = false) {
...
void print_data(
    std::variant<int64_t, float, std::string, bool, std::vector<joy_object *>>
        data,
    joy_object *o, bool print_space = true) {
  switch (data.index()) {
  case 0:
